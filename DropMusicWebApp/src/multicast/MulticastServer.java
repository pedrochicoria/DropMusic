...
public class MulticastServer extends Thread{
    ...
    private void readRequests(MulticastSocket receive_socket, MulticastSocket send_socket){
        ...
        try {
            ...
            while (true) {
                ...
                switch(data[0]){
                    case "type|dropbox_login":
                        String u = dropboxLogin(message);
                        send_packet = new DatagramPacket(u.getBytes(),u.getBytes().length,group,RMI_PORT);
                        send_socket.send(send_packet);
                        break;
                    case "type|dropbox_association":
                        boolean sended;

                        sended = dropboxAssociate(message);
                        if(sended){
                            String message_to_rmi = "type|dropbox_association;status|on";
                            send_packet = new DatagramPacket(message_to_rmi.getBytes(),message_to_rmi.getBytes().length,group,RMI_PORT);
                            send_socket.send(send_packet);

                        }else{
                            String message_to_rmi = "type|dropbox_association;status|off";
                            send_packet = new DatagramPacket(message_to_rmi.getBytes(),message_to_rmi.getBytes().length,group,RMI_PORT);
                            send_socket.send(send_packet);
                        }
                        break;
                    ...
                    case "type|details_review":
                        ...
                        break;
                    case "type|albumRating":
                        String rating = albumRating(message);
                        send_packet = new DatagramPacket(rating.getBytes(),rating.getBytes().length,group,RMI_PORT);
                        send_socket.send(send_packet);
                        break;
    ...
    private String dropboxLogin(String message_from_rmi){
        ...
        for(User user:users){
            if(user.getId().equals(id)){
                return "type|dropbox_login;username|"+user.getUsername();
            }
        }
        return null;
    }

    private boolean dropboxAssociate(String message_from_rmi){
        ...
        String [] aux_id = data[2].split("\\|");
        String id = aux_id[1];

        String [] aux_token = data[3].split("\\|");
        String token = aux_id[1];
        ...
        if(!artists.isEmpty()){
            for(Artist a : artists){
                if(a.getArtist_name().equals(artist)){
                    ...
                    System.out.println(Integer.toString(a.getAlbums().size()));
                    System.out.println(message_to_rmi);
                    for(Album album : a.getAlbums()){
                        aux += ";album_" + Integer.toString(counter) + "|" + album.getAlbum_name();
                        counter ++;
                    }
                    return message_to_rmi + aux;
                }
            }
    ...
    private String albumRating(String message_from_rmi){
        String [] data = message_from_rmi.split(";");
        ...
        for(Artist artist : artists){
                ...
                for(Album album : artist.getAlbums()){
                    if(album.getAlbum_name().equals(album_name)){
                        ...
                        if(counter > 0){
                            return Integer.toString(res/counter);
                        }
                    }
                }
